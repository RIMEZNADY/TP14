...
class Service {
    ...
    /**
     * Helper function to safely get property as String from SoapObject
     */
    private fun SoapObject.getPropertySafelyAsString(propertyName: String): String? {
        return try {
            val property = this.getProperty(propertyName)
            when (property) {
                is String -> property
                is SoapObject -> property.getProperty(0)?.toString()
